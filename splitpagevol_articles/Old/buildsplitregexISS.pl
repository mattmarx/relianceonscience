...
open(INFILE,"wosplpubinfo1955-2017_filteredISS.txt");
...
#print STDERR "title = >$title<\n";
	...
	if (!(($firstauthor eq "")||($firstauthor eq "[anonymous]")||($year<1900)||($year>2017)||($vol eq "")||($issue eq "")||($firstpage eq ""))) {
         $regex= "\tif (/\\W$firstauthor\\W/ && /\\D$vol\\D+$issue\\D+$firstpage\\D/) { print \"$wosid\t\t$year\t$vol\t$issue\t$firstpage\t$firstauthor\t$title\t\$_\"; }\n";	  
