...
if ($ARGV[0]=~/^wos$/i) {
    ...
    if (!$inputyear) {
	die("Usage: buildsplitregex_byyear.pl mag YEAR|wos YEAR|filename_or_fullpath_of_file\n");
    }
}
elsif ($ARGV[0]=~/^pubmed$/i) {
    $inputfilesbasepath="/projectnb/marxnsf1/dropbox/bigdata/nplmatch/inputs/pubmed/pubmedbyyear/pubmed_";
    $inputyear=$ARGV[1];
    $sourcefilecode="pubmed";

    $inputfile="$inputfilesbasepath"."$inputyear".".tsv";

    if (!$inputyear) {
	die("Usage: buildsplitregex_byyear.pl pubmed YEAR|wos YEAR|filename_or_fullpath_of_file\n");
    }
}
else {
    $inputfile=$ARGV[0];
...
$outputdir="/projectnb/marxnsf1/dropbox/bigdata/nplmatch/splitpagevol_articles/year_regex_scripts_body_" . "$sourcefilecode". "/";;

$inputdir="/projectnb/marxnsf1/dropbox/bigdata/nplmatch/splitpagevol_patent/body/";
...
if ($file==0) {
    # Perl line 
    $template_contents="#!/share/pkg/perl/5.24.0/install/bin/perl\n\n";
	...
	for($digit=1;$digit<10;$digit++) {
		...
		if ($sizect>=($filesize/8)) { # Because the comparison on the other side for single digits is also much larger, treat the file size limit for them as being 1/8 as big as for other numbers.
		    ...
		    $filenum=$filect+1000;
		    if ($filect!=0) { 
			if (($digit!=1)||($firstyear==1)) { # If new digit set, files will already be closed
			    print OUTFILE "}\n";
