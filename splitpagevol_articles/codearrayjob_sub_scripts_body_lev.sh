...
do
 ...
 then
  ...
  buildarrayscript.pl splitpagevol body mag_lev $year
  qsub arrayjob_sub_scripts/array-$year.sh
 else
  echo "submitting single job"
  qsub -P marxnsf1 -N runcode_body_mag_lev$year -o year_regex_output_body_mag_lev/year$year-1000.txt -b y year_regex_scripts_body_mag_lev/year$year-1000.pl
 fi
done
