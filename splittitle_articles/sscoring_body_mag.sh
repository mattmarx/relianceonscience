...
do
 ...
 echo
 echo "doing $YEAR."
 FILESFORYEAR=$(ls -l year_regex_output_${FRONTORBODY}_${ARTICLEDB}_lev/year${YEAR}-* | wc -l)
 ...
 echo "" >> $TEMPSLURMFILE
 echo "perl $NPL_BASE/nplmatch/process_matches/score_matches.pl $NPL_BASE/nplmatch/split${SPLITTYPE}_articles/year_regex_output_${FRONTORBODY}_${ARTICLEDB}_lev/year${YEAR}-\$FILE_ID.txt -body > $NPL_BASE/nplmatch/split${SPLITTYPE}_articles/year_regex_scored_${FRONTORBODY}_${ARTICLEDB}_lev/year${YEAR}-\$FILE_ID.txt" >>$TEMPSLURMFILE
