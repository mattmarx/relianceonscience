...
if ($ARGV[0]=~/^wos$/i) {
    ...
    if (!$inputyear) {
	die("Usage: buildtitleregex_byyear.pl [mag YEAR]|[wos YEAR]|[pubmed YEAR}[filename_or_fullpath_of_file]\n");
    }
}
elsif ($ARGV[0]=~/^mag$/i) {
...
$outputdir="/projectnb/marxnsf1/dropbox/bigdata/nplmatch/splittitle_articles/year_regex_scripts_body_" . "$sourcefilecode". "/";
