...
if ($ARGV[0]=~/^wos$/i) {
    ...
    if (!$inputyear) {
	die("Usage: buildtitleregex_byyear_body_lev.pl [mag YEAR]|[wos YEAR]|[pubmed YEAR}[filename_or_fullpath_of_file]\n");
    }
}
elsif ($ARGV[0]=~/^mag$/i) {
...
$outputdir="/projectnb/marxnsf1/dropbox/bigdata/nplmatch/splittitle_articles/year_regex_scripts_body_" . "$sourcefilecode" . "_lev/";

$inputdir="/projectnb/marxnsf1/dropbox/bigdata/nplmatch/splittitle_patent/body/";
...
if ($file==0) {
    ...
    open(PERLFILE,"/projectnb/marxnsf1/dropbox/bigdata/nplmatch/splittitle_articles/template_perl_body_ld")||die("Can't open Perl Template file template_perl_body_ld\n");
    $template_contents="";
    while(<PERLFILE>) {
	$template_contents.="$_";
    }
